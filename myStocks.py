...
def main():
    ...
    if job=='선택':
        ...
        if chk0: st.markdown('''
                    ...
                    ###### :green[--- 2023년 1월9일 현재 바닥을 확인후 반등중임 ---]
                    ###### :violet[기술적 지표(rsi(10): 침체구간(30이하), 볼린저밴드(40,2): 하단선 하향 돌파)]
                    ##### :orange[주식시장의 저평가: 0.9이하 + 기술적 저점일때 적극적으로 매매]
                    ##### :orange[위의 3가지 요건이 만족되면 몰빵한다.]
                    ###### :violet[일반적 통계를 활용한 매매 방법]
                    ###### :violet[1월 진입 3,4월 매도(코스닥이 코스피보다 좋다, 개별주(중소형주)위주로 매매)]
                    ###### :violet[1월~3월 매수, 4월에 매도]
                    ###### :violet[장단기 금리차 역전이 해소 된 후 3~7개월내에 폭락장 나올 수 있음]
                    ###### :violet[5월~10월은 폭락장이 나올 수 있으므로 콥버스 매수하거나, 조심해야 된다]
                    ###### :violet[폭락장 나오면 매수 후 12월까지 보유하면 큰 수익이 가능할 것으로 생각된다]
                ''')        
