...
def main():
    ...
    # Side Bar 생성
    job=st.sidebar.selectbox('선택',['선택','관심주','조회','특징주','시장 상황 확인','년도별 가격 변동률 조회','체크 리스트',\
                             '가격 변동률(년간)'])
    ...
    if job=='조회':
        ...
        종료일=str(조회일).replace('-','')
        조회선택=st.sidebar.selectbox('선택',['선택','업종별 등락률 순위','배당농부법 종목1','적정주가와 괴리율 보기','코스피200','매출증가 종목 보기','인덱스종류'])

        if 조회선택=='업종별 등락률 순위':
            Display.업종별등락률순위보기(종료일)
            return       
