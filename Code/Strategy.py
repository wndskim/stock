...
def 현장세_보기():
    ...
    st.text('추세는 여전히 하락 중이나 2023년 1월, 2월은 단기 상승 후 3,4월 고점을 형성 후 다시 하락 할것으로 예상 된다.')
    st.text('2023년 1분기 전략')
    st.text('1월, 2월 매수/보유(이번 반등장은 코스닥 중.소형주 위주로 매매)')
    st.text('3월, 4월 매도')
    st.text('5월~10월은 폭락장이 나돌수 있다')
    st.text('대응은 곱버를 매수 하거나 매매를 쉰다(매우 조심해야 한다)')
