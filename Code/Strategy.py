...
def 급등예상주발굴매매법():
    ...
    st.markdown('''
            ...
            ###### :blue[-- 외국인, 투신 매매동향, 수급분석, 금리.환율.선물.뉴스등을 통해 시장 변화를 파악한다.]
            ###### :blue[-- 업종별, 대형/중형/소형에서 가장 빠르고 크게 움직이는 종복 검색(동종비교, 상대 우위성)]
            ###### :blue[2. 매수 시그널 파악]
            ###### :blue[-- 월,주,일,분,30분봉을 차례로 확인하여 5일 평균선 돌파 종목]
            ###### :blue[3. 유동성 장세인 경우(큰 시세가 나는 장)]
            ###### :blue[-- 피날레는 결국 대형주가 한다.]
            ###### :blue[4. 조정장세인 경우]
            ###### :blue[-- 테마주, 중소형주, 스몰캡, 저가중등 특히 순환매 주의]
            ''')

    st.markdown('''
            ##### :orange[-- 급등 예상주 발굴 매매법 --]
            ###### :blue[1. 이평선 정배열하의 점프 종목]
            ###### :blue[2. 신고가를 가볍게 돌파(대기 매물대를 가볍계 돌파하는 종목) => 강력한 매수 세력 존재 의미]
            ###### :blue[3. 2중,3중의 바닥을 다진 종목 중 이평선 정배열 전환 종목(저향선을 모두 뚫었다는 의미)]
            ###### :blue[4. 주가가 60일, 120일선을 돌파 후 2달이상 빠지지 않는 종목 ****]
            ###### :blue[5. 역배열하의 이격큰 종목, 즉 장기하락 중 단기급락주(골이 깊으면 산이 높다)]
            ###### :blue[6. V자 바닥형(급락후 매도세력 전멸로 일시적인 매물 공백상태)-되돌림/목눌림 구분 필요]
            ###### :blue[7. 거래량 연중 최저에서 방향을 바꾸거나 주가가 하락할수록 거래량이 늘어나는 종목]
            ###### :blue[8. 장기간 횡보하고 최근 6개월 거래량 최저점 기록한 종목]
            ###### :blue[9. 매물 부담이 적고 전고점 거래가격대와 괴리도가 큰 종목이 거래량이 늘어 나면서 상승]
            ###### :blue[** 편승: 거래량 급증하며 상승시 매물 저항이 크지 않은 경우]
            ###### :blue[** 하차: 거래량 급증하며 사승 하였지만 상승폭이 작거나 빠지면서 3일연속 거래량 지속(세력실패)]
            ''')
