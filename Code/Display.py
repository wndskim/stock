...
def 매출증가_종목보기(날짜):
    ...
    if len(df_위치)>0:
        ...
        # 일자별 시가총액 조회
        시총=GetData.종목별_시가총액_기간(시작일,종료일,_dict[종목])
        시총['시가총액']=시총['시가총액']/100000000 # 억단위로 조정
        시총['시가총액']=시총['시가총액'].astype(int)
...
def 거래량폭증_종목보기(조회일):
    ...
    if radio=='발굴된 종목 보기':
        ...
        col1,col2=st.columns([1,4])
        with col1:
            radio=st.radio('선택',('20일 평균거래량 10배이상','20일 평균거래량 5배이상','120이평 상승중 종목','240이평 상승중 종목','30주 골든크로스','20/60 골든크로스','RSI30이하','RSI70이상'))
