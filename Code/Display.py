...
def 관심주_보기(티커, 종목, 상승파동비율, 위치정보):
    ...
    with col1:
        ...
        st.text(가격+'\n'+내재가치+파동001값+파동007값+파동014값+파동021값+파동025값+파동382값+파동050값+파동618값+파동832값+파동100값+파동1382값+파동1618값+파동200값)
    ...
    with col2:
        ...
        현재가값='현재가: '+str(현재가)+'\n'
        최고가52값='현재가: '+str(최고가52)+'\n'
        최저가52값='현재가: '+str(최저가52)+'\n'
        위치1값='현재가: '+위치1+'\n'
        위치2값='현재가: '+위치2+'\n'

        st.text(현재가값+최고가52값+최저가52값+위치1값+위치2값)
