...
def 거래량폭증_종목보기(조회일):
    ...
    radio=st.radio('선택',('발굴하기','발굴된 종목 보기'))
    if radio=='발굴된 종목 보기':
        folder='./Data/'
        작업파일='관심주_기타.xlsx'
    ...
    # 발굴하기
        ...
        with col2:
            ...
            st.text('기준일: '+str(날짜)+'   업종: '+업종+'   업종순위: '+str(업종순위)+'   '+대중소,'링크: '+링크)
            st.dataframe(df_링크)
...
def 재무정보_보여주기(조회일, 시작일, 종료일, 티커, 종목):
    ...
    with col4:
        try:
            ...
            종목_per,업종_per,시가총액,발행주식수,영업이익,EPS=GetData.적정주가계산용자료가져오기(티커)
            ...
            if math.isnan(영업이익):
                ...
                적정주가2=int(영업이익*100000000*업종_per/발행주식수)
                적정주가2값='적정주가2: '+str(적정주가2)+' <= 예상영업이익/업종PER 기준'
            ...
            # if 적정주가1!='N/A' and 적정주가2!='N/A':
            #     적정주가괴리율1=종가/적정주가1.round(2)
            #     적정주가괴리율2=종가/적정주가2.round(2)
            # else:
            #     적정주가괴리율1='N/A'
            #     적정주가괴리율2='N/A'
