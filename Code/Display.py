...
def 참조링크보기(티커,종목):
    ...
    # 'https://comp.fnguide.com/SVO2/ASP/SVD_Main.asp?pGB=1&gicode=A124500&cID=&MenuYn=Y&ReportGB=&NewMenuID=101&stkGb=701'

    st.write('[NICE CompanySearch](https://comp.kisline.com/hi/HI0100M010GE.nice?stockcd={}&nav=1)'.format(종목))
    st.write('[CompanyGuide](https://comp.fnguide.com/SVO2/ASP/SVD_Main.asp?pGB=1&gicode={}&cID=&MenuYn=Y&ReportGB=&NewMenuID=101&stkGb=701)'.format(종목))
...
def 거래량폭증_종목보기(조회일):
    ...
    if radio=='발굴된 종목 보기':
        ...
        with col1:
            radio=st.radio('선택',('20일 평균거래량 10배이상','20일 평균거래량 5배이상','120이평 상승중 종목','240이평 상승중 종목','30주 골든크로스','기타'))
