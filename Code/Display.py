...
def 거래량폭증_종목보기(조회일):
    ...
    if radio=='발굴된 종목 보기':
        ...
        with col1:
            radio=st.radio('선택',('20일 평균거래량 10배이상','20일 평균거래량 5배이상','120일평 상승중 종목','기타'))
            if radio=='20일 평균거래량 5배이상': df=df[(df['거래량20대비']>4.99) & (df['거래량20대비']<10)]
            elif radio=='20일 평균거래량 10배이상': df=df[df['거래량20대비']>9.99]
            # elif radio=='120일평 상승중 종목': df=df[df['상태120']=='120이평 상승중']
            elif radio=='120일평 상승중 종목': df=df.query('상태120=="120이평 상승중"')  # part_states.query('index >= 2020 and index < 2022')[state]
